...
namespace Stock.UI.Views
{
    ...
    /// </summary>
    public partial class OwnerTableView : ITableView
    {
        ...
        {
            var dialog = new OwnerAddView { Owner = Window.GetWindow(this) };
        ...
        {
            var item = ViewModel.SelectedItem as Owner;
            if (item != null)
            {
                var dialog = new OwnerAddView(item) { Owner = Window.GetWindow(this) };
                dialog.Closed += (s, e) => dialog.Owner.Focus();
                dialog.Show();
            }
        }
        ...
        private void LoadSettings()
        {
            ...
            {
                var columnWidth = string.Format("OwnerTableColumn{0}Width", i);
                DataGrid.Columns[i].Width =
                    new DataGridLength(AppSettings.GetAsDouble(columnWidth));

                var columnDisplayIndex = string.Format("OwnerTableColumn{0}DisplayIndex", i);
        ...
        private void OwnerTableView_OnLoaded(object sender, RoutedEventArgs e)
        {
